...
module.exports.changeMarketingStatus = async (email) => {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      getLeadCampaignDataExtension(mcClient, email, async (err, response) => {
        if (err) {
          console.log(err);
          reject(err);
        } else {
          ...
          for (let i = 0; i < result.Results.length; i++) {
            ...
            if (campaign && campaign.Value) {
              ...
              await updateEmailPermission(mcClient, email, campaign.Value);
            }
          }
    ...
    const email = await sendEmailToCustomer(email);
